...
const palette = {
  ...
  orange: {
    icon: "from-orange-500/25 to-amber-500/10 text-orange-400 ring-orange-500/40",
    hairline: "via-orange-500/70",
    glow: "bg-orange-500/10",
  },
  pink: {
    icon: "from-pink-500/25 to-rose-500/10 text-pink-400 ring-pink-500/40",
    hairline: "via-pink-500/70",
    glow: "bg-pink-500/10",
  },
  red: {
    icon: "from-red-500/25 to-rose-500/10 text-red-400 ring-red-500/40",
    hairline: "via-red-500/70",
    glow: "bg-red-500/10",
  },
};

export default function AdminStatsCard({ title, value, icon, color = "blue" }) {
  const tint = palette[color] || palette.blue;

  return (
    <div className="card group relative overflow-hidden p-5 sm:p-6 hover:-translate-y-1 transition-all duration-300">
      ...
      <h2 className="relative mt-4 text-2xl font-bold text-ink sm:text-3xl lg:text-4xl break-words">
        {value}
      </h2>
    </div>
  );
