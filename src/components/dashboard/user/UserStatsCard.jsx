...
const palette = {
  indigo: {
    icon: "from-indigo-500/25 to-violet-500/10 text-indigo-400 ring-indigo-500/40",
...
export default function UserStatsCard({ title, value, icon, color = "indigo" }) {
  const tint = palette[color] || palette.indigo;
  ...
  return (
    <div className="card group relative overflow-hidden p-5 hover:-translate-y-1 transition-all duration-300">
      ...
      <h2 className="relative mt-4 text-2xl font-bold text-ink sm:text-3xl break-words">
        {value}
      </h2>
    </div>
  );
